...
namespace panda::guard {
    struct FileNameCacheInfo {
        std::map<std::string, std::string> identifierCacheMap_;
        std::map<std::string, std::string> memberMethodCacheMap_;
        std::string obfName_;
        std::string oriSourceFile_;
        std::string obfSourceFile_;
    };

    struct ProjectNameCacheInfo {
        std::map<std::string, FileNameCacheInfo> fileCacheInfoMap_;
        std::string entryPackageInfo_;
        std::string compileSdkVersion_;
        std::map<std::string, std::string> propertyCacheMap_;
        std::map<std::string, std::string> fileNameCacheMap_;
    };
    ...
    public:
        ...
        void AddObfIdentifierName(const std::string &filePath, const std::string &origin,
                                  const std::string &obfName);

        void AddObfMemberMethodName(const std::string &filePath, const std::string &origin,
                                    const std::string &obfName);
    ...
    private:
        void ParseHistory(const std::string &content);
