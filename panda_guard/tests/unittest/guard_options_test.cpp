...
HWTEST(GuardOptionsUnitTest, guard_options_test_001, TestSize.Level4)
{
    ...
    EXPECT_EQ(options.EnableExport(), true);
    EXPECT_EQ(options.EnableRemoveLog(), true);
    EXPECT_EQ(options.GetPrintNameCache().empty(), true);
    EXPECT_EQ(options.GetApplyNameCache().empty(), true);
    EXPECT_EQ(options.EnableProperty(), true);
    EXPECT_EQ(options.EnableToplevel(), true);
    EXPECT_EQ(options.EnableFileName(), true);
    ...
    EXPECT_EQ(options.IsKeepPath(), false);
    EXPECT_EQ(options.IsKeepPath("xxx"), true);
...
HWTEST(GuardOptionsUnitTest, guard_options_test_003, TestSize.Level4)
{
    ...
    EXPECT_EQ(options.DisableObfuscation(), false);
    EXPECT_EQ(options.EnableExport(), false);
    EXPECT_EQ(options.EnableRemoveLog(), false);
    EXPECT_EQ(options.EnableProperty(), false);
    EXPECT_EQ(options.EnableToplevel(), false);
    EXPECT_EQ(options.EnableFileName(), false);
    EXPECT_EQ(options.IsKeepPath(), false);
    guard::TestUtil::RemoveFile(TEMP_OPTIONS_TEST_JSON_FILE);
}
...
HWTEST(GuardOptionsUnitTest, guard_options_test_005, TestSize.Level4)
{
    ...
    // ../x/.*/z.ets
    EXPECT_EQ(options.IsKeepPath("../x/abc/z.ets"), true);
    EXPECT_EQ(options.IsKeepPath("../a/b/z.ets"), false);
    // ../x/./z.ets
    EXPECT_EQ(options.IsKeepPath("../x/y/z.ets"), false);
    EXPECT_EQ(options.IsKeepPath("../x/abc/z.ets"), true);
}
