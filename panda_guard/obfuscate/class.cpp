...
void panda::guard::Class::Build()
{
    ...
    this->constructor_.insInfo_ = this->insInfo_;
...
{
    this->ForEachFunction([&callback](Function &func) {
        func.ForEachIns(callback);
    });
}
...
void panda::guard::Class::CreateMethod(const pandasm::LiteralArray &literalArray, size_t index, bool isStatic)
{
    ...
    method.insInfo_ = this->insInfo_;
    method.scope_ = this->scope_;
    method.export_ = this->export_;

    method.Init();
    method.Create();
    PANDA_GUARD_ASSERT_PRINT(method.name_ != methodName, TAG << "get different name for method:" << method.idx_);
