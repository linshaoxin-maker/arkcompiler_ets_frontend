...
void panda::guard::Node::Build()
{
    ...
    this->isNormalizedOhmUrl_ = StringUtil::IsPrefixMatched(name_, NORMALIZED_OHM_DELIMITER.data());
    CreateFilePath();
    ...
    entryFunc.ForEachIns([&](const InstructionInfo &info) -> void {
        CreateFunction(info, TOP_LEVEL);
        CreateClass(info, TOP_LEVEL);
        CreateOuterMethod(info);
        CreateObject(info, TOP_LEVEL);
...
void panda::guard::Node::CreateFunction(const InstructionInfo &info, const Scope scope)
{
    ...
    function.ForEachIns([&](const InstructionInfo &insInfo) -> void {
        CreateFunction(insInfo, FUNCTION);
        CreateClass(insInfo, FUNCTION);
        CreateOuterMethod(insInfo);
        CreateObject(insInfo, FUNCTION);
        CreateObjectOuterProperty(insInfo);
    });
...
void panda::guard::Node::CreateObject(const InstructionInfo &info, const Scope scope)
{
    ...
    LOG(INFO, PANDAGUARD) << TAG << "find record object:" << object.literalArrayIdx_;
...
void panda::guard::Node::CreateObjectOuterProperty(const InstructionInfo &info)
{
    ...
    LOG(INFO, PANDAGUARD) << TAG << "find outer property:" << propertyName;
}
...
void panda::guard::Node::CreateFilePathForNormalizedMode()
{
    ...
    PANDA_GUARD_ASSERT_PRINT(!StringUtil::IsPrefixMatched(name_, prefix), TAG << "invalid normalizedOhmUrl prefix");
    filepath_.prePart = std::move(prefix);

    size_t pos = name_.find_first_of(NORMALIZED_OHM_DELIMITER.data(), filepath_.prePart.size());
    PANDA_GUARD_ASSERT_PRINT(pos == std::string::npos, TAG << "invalid normalizedOhmUrl format");
    filepath_.name = name_.substr(filepath_.prePart.size(), pos - filepath_.prePart.size());

    filepath_.postPart = name_.substr(pos, name_.size() - pos);
}
...
void panda::guard::Node::RefreshNeedUpdate()
{
    ...
    if (GuardContext::GetInstance()->GetGuardOptions()->IsKeepPath(this->name_)) {
        LOG(INFO, PANDAGUARD) << TAG << "find keep rule for:" << this->name_;
...
void panda::guard::Node::Update()
{
    ...
    for (auto &[_, function]: this->functionTable_) {
        function.Obfuscate();
        function.WriteNameCache(this->name_);
    }

    for (auto &[_, clazz]: this->classTable_) {
        clazz.Obfuscate();
        clazz.WriteNameCache(this->name_);
    }

    if (this->contentNeedUpdate_) {
        moduleRecord_.Obfuscate();
        moduleRecord_.WriteNameCache(this->name_);

        for (auto &[_, object]: this->objectTable_) {
            object.Obfuscate();
            object.WriteNameCache(this->name_);
        }
    }

    this->WriteFileCache(this->name_);
...
{
    GuardContext::GetInstance()->GetNameCache()->AddObfName(filePath, this->obfName_);
