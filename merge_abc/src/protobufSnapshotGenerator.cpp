...
void ProtobufSnapshotGenerator::GenerateSnapshot(const panda::pandasm::Program &program, const std::string &outputName)
{
    ...
    std::fstream output(ConvertPlatFormPath(outputName), std::ios::out | std::ios::trunc | std::ios::binary);
    if (!output) {
...
{
    std::fstream input(ConvertPlatFormPath(inputName), std::ios::in | std::ios::binary);
    if (!input) {
    ...
    const bool &isDebug, const std::string &cacheFilePath)
{
    ...
    std::fstream output(ConvertPlatFormPath(cacheFilePath), std::ios::out | std::ios::trunc | std::ios::binary);
    if (!output) {
...
{
    std::fstream input(ConvertPlatFormPath(cacheFilePath), std::ios::in | std::ios::binary);
    if (!input) {
