    ...
    switch (childNode->Type()) {
        case ir::AstNodeType::IDENTIFIER: {
            ...
            if (InClass()) {
                ...
                if (classDefinition != nullptr && GetClassAliasName(name) != name) {
                    ident->SetName(GetClassAliasName(name));
                }
            }
        ...
        case ir::AstNodeType::CLASS_DECLARATION: {
            ...
            util::StringView aliasName = VisitClassDecorators(node);
            DuringClass duringClass(&classList_, node->Definition()->GetName(), aliasName);
        ...
        case ir::AstNodeType::CLASS_EXPRESSION: {
            ...
            DuringClass duringClass(&classList_, node->Definition()->GetName(), node->Definition()->GetName());
...
{
    binder::Scope *scope = Scope();
    while (scope != nullptr)
    {
        binder::Variable *v = scope->FindLocal(name, binder::ResolveBindingOptions::BINDINGS);
        if (v != nullptr) {
...
util::StringView Transformer::VisitClassDecorators(ir::ClassDeclaration *node)
{
