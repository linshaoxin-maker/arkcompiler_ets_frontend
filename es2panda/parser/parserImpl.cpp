...
ir::Expression *ParserImpl::ParseComputedPropetryName(binder::EnumDecl *&decl, const lexer::SourcePosition &keyStartLoc, 
                                                      bool isDeclare) 
{
                                                    ...
                                                    bool isExport, bool isDeclare, bool isConst)
{
    ...
    while (lexer_->GetToken().Type() != lexer::TokenType::PUNCTUATOR_RIGHT_BRACE) {
        ...
        if (lexer_->GetToken().Type() == lexer::TokenType::LITERAL_IDENT) {
            ...
            memberKey = ParseComputedPropetryName(decl, keyStartLoc, isDeclare);
        } else {
            ThrowSyntaxError("Unexpected token in enum member");
        }
