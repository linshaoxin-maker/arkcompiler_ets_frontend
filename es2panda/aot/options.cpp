...
void Options::CollectInputSourceFile(const std::vector<std::string> &itemList, const std::string &inputExtension)
{
    ...
    constexpr uint32_t ORIGIN_SOURCE_LANG = 6;
    ...
    // older version do not have origin source lang, use ECMASCRIPT as default value
    if (itemList.size() > ORIGIN_SOURCE_LANG) {
        src.sourceLang = itemList[ORIGIN_SOURCE_LANG];
    }
