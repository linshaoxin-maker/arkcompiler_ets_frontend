...
void EmitFileQueue::Schedule()
{
    ...
    auto targetApiSubVersion = options_->CompilerOptions().targetApiSubVersion;

    if (mergeAbc_) {
        // generate merged abc
        auto emitMergedAbcJob = new EmitMergedAbcJob(options_->CompilerOutput(),
            options_->CompilerOptions().transformLib, progsInfo_, targetApi, targetApiSubVersion);
...
void EmitMergedAbcJob::Run()
{
    ...
    bool success = panda::pandasm::AsmEmitter::EmitPrograms(
        panda::os::file::File::GetExtendedFilePath(outputFileName_),
        progs, true, targetApiVersion_, targetApiSubVersion_);
