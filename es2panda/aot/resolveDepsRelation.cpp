    ...
    try {
        for (auto recordName : compileContextInfo_.compileEntries) {
            for (auto &[key, value] : progsInfo_) {
                CollectCompileEntryKey(recordName, key, &value->program);
            }
        }
...
void ResolveDepsRelation::CollectCompileEntryKey(std::string recordName, std::string programKey,
                                                 panda::pandasm::Program *program)
{
    for (auto &[recordKey, record] : program->record_table) {
        if (recordKey == recordName) {
            if (resolveDepsRelation_->find(programKey) == resolveDepsRelation_->end()) {
                std::unordered_set<std::string> depsSet{};
                resolveDepsRelation_->insert(std::pair<std::string, std::unordered_set<std::string>>(programKey, depsSet));
            }
            CollectRecord(recordName, programKey);
            break;
        }
    }
}
...
void ResolveDepsRelation::CollectRecordDepsRelationLiterralValue(panda::pandasm::LiteralArray::Literal literal,
                                                    ...
                                                    std::string compileEntryKey)
{
    ...
    for (auto &literalarrayPair : program->literalarray_table) {
        ...
        for (auto& literal : literalarrayPair.second.literals_) {
            CollectRecordDepsRelationLiterralValue(literal, compileEntryKey);
        }
    }
    // dynamic collection
