...
{
    if (ohmurl.find("@normalized:N") != std::string::npos && !IsHspPackage(ohmurl, compileContextInfo_.hspPkgNames)) {
        return true;
    }
    return false;
}
...
void DepsRelationResolver::CollectStaticImportDepsRelation(const panda::pandasm::Program &program, const std::string &recordName)
{
    for (auto &literalarrayPair : program.literalarray_table) {
        if (literalarrayPair.first.find(recordName) == std::string::npos) {
            continue;
        }
