...
void ClassDefinition::CalculateClassExpectedPropertyCount()
{
    ...
    auto *ctorFunc = Ctor()->Value()->Function();
                                             ...
                                             const std::function<void(const util::StringView&)>& addPropertyName)
{
    for (const auto &stmt : body->Statements()) {
        ...
        if (memberExpr->Object()->IsThisExpression()) {
            ProcessMemberExpressionWithThis(memberExpr, addPropertyName);
        }
    }
}

void ClassDefinition::ProcessMemberExpressionWithThis(
    const MemberExpression *memberExpr,
    const std::function<void(const util::StringView&)>& addPropertyName)
{
...
void ClassDefinition::ProcessPropertyKey(const Expression* key,
                                         const std::function<void(const util::StringView&)>& addPropertyName)
{
    // Private properties are not processed here because the private node is a privateIdentifier
    if (key->IsIdentifier()) {
