...
void PandaGen::StoreVar(const ir::AstNode *node, const binder::ScopeFindResult &result, bool isDeclaration)
{
    ...
    if (var->Declaration()->IsLetOrConstDecl() && result.scope->IsGlobalScope()) {
...
void PandaGen::StoreLexicalVar(const ir::AstNode *node, uint32_t level, uint32_t slot, VReg destVreg)
{
