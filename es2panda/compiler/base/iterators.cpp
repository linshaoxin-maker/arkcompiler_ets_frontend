...
void Iterator::CallMethodWithValue(VReg value) const
{
    std::vector<VReg> args = {method_, iterator_, value};
    pg_->CallThis(node_, args);
}

void Iterator::CallMethod() const
{
    std::vector<VReg> args = {method_, iterator_};
    pg_->CallThis(node_, args);
}
...
void Iterator::CloseWithException() const
{
    if (type_ == IteratorType::SYNC) {
        RegScope rs(pg_);
        VReg exception = pg_->AllocReg();
        VReg constReg = pg_->AllocReg();
        Label *noReturn = pg_->AllocLabel();
        pg_->StoreAccumulator(node_, exception);

        pg_->LoadConst(node_, Constant::JS_TRUE);
        pg_->StoreAccumulator(node_, constReg);
        Complete();
        pg_->Condition(node_, lexer::TokenType::PUNCTUATOR_NOT_STRICT_EQUAL, constReg, noReturn);

        // close iterator
        pg_->LoadObjByName(node_, iterator_, "return");
        pg_->StoreAccumulator(node_, nextResult_);
        pg_->LoadConst(node_, Constant::JS_UNDEFINED);
        pg_->Condition(node_, lexer::TokenType::PUNCTUATOR_NOT_STRICT_EQUAL, nextResult_, noReturn);
        std::vector<VReg> args = {nextResult_, iterator_};
        pg_->CallThis(node_, args);
    ...
    // support IteratorType::ASYNC later
}
