...
def get_args():
    ...
    parser.add_argument(
        ...
        help='capture stderr')
    parser.add_argument(
        '--abc-to-asm', '-abc2asm', action='store_true', dest='abc_to_asm',
...
class AbcToAsmRunner(Runner):
    def __init__(self, args):
        Runner.__init__(self, args, "Abc to asm")
...
class AbcToAsmTest(Test):
    def run(self, runner):
        ...
        self.log_cmd.append(abc_to_asm_cmd)
        process_gen_abc = subprocess.Popen(
            gen_abc_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        gen_abc_out, gen_abc_err = process.communicate()
        gen_abc_output = gen_abc_out.decode("utf-8", errors="ignore") + gen_abc_err.decode("utf-8", errors="ignore")
        process_abc_to_asm = subprocess.Popen(
            abc_to_asm_cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        abc_to_asm_out, abc_to_asm_err = process.communicate()
...
def main():
    ...
    if args.abc_to_asm:
        ...
        runner.add_directory("abc2asm/js", "js" , ["--dump-asm-program"])
        runner.add_directory("abc2asm/ts", "ts", ["--dump-asm-program"])
        runner.add_directory("abc2asm/abc", "abc", ["--enable-abc-input", "--dump-asm-program"])
        runners.append(runner)
