...
const tscKind2es2pandaAST = {
    ...
    BooleanKeyword:"TSBooleanKeyword",ArrayType:"TSArrayType",MethodDeclaration:"MethodDefinition",ThisKeyword:"ThisExpression",
    NumericLiteral:"NumberLiteral", ArrayLiteralExpression:"ArrayExpression",ObjectLiteralExpression:"ObjectExpression",TypeQuery:"TSTypeQuery",
    TypeLiteral:"TSTypeLiteral",ConstructSignature:"TSConstructSignatureDeclaration",VoidKeyword:"TSVoidKeyword",TypeParameter:"TSTypeParameter",
    StringKeyword:"TSStringKeyword",IndexSignature:"TSIndexSignature",ModuleBlock:"TSModuleBlock", ModuleDeclaration:"TSModuleDeclaration",
    MethodSignature:"TSMethodSignature",FunctionType:"TSFunctionType",AnyKeyword:"TSAnyKeyword",ElementAccessExpression:"MemberExpression",
    PrefixUnaryExpression:"UnaryExpression",ArrowFunction:"ArrowFunctionExpression",CaseClause:"SwitchCase",DefaultClause:"SwitchCase", ForStatement:"ForUpdateStatement",
    DoStatement:"DoWhileStatement",NullKeyword:"NullLiteral",TrueKeyword:"BooleanLiteral",PropertyAssignment:"Property",LabeledStatement:"LabelledStatement",
    PostfixUnaryExpression:"UpdateExpression",TypeAssertionExpression:"TSTypeAssertion",CallSignature:"TSCallSignatureDeclaration",UnionType:"TSUnionType",
    TypeAliasDeclaration:"TSTypeAliasDeclaration",ObjectBindingPattern:"ObjectPattern",BindingElement:"Property",AsExpression:"TSAsExpression",ArrayBindingPattern:"ArrayPattern",
    EnumDeclaration:"TSEnumDeclaration",EnumMember:"TSEnumMember",VoidExpression:"UnaryExpression",TypeOfExpression:"UnaryExpression",UndefinedKeyword:"TSUndefinedKeyword",
    ParenthesizedType:"TSParenthesizedType",NonNullExpression:"TSNonNullExpression",TypeOperator:"TSTypeOperator",LiteralType:"TSLiteralType",UnknownKeyword:"TSUnknownKeyword",
    NeverKeyword:"TSNeverKeyword",TupleType:"TSTupleType", IndexedAccessType:"TSIndexedAccessType",PrivateIdentifier:"TSPrivateIdentifier",
    TypePredicate:"TSTypePredicate",FirstTypeNode:"TSTypePredicate",TemplateExpression:"TemplateLiteral",TemplateHead:"TemplateElement",
    ShorthandPropertyAssignment:"Property",LastStatement:"DebuggerStatement",DebuggerStatement:"DebuggerStatement",ImportEqualsDeclaration:"TSImportEqualsDeclaration"
}
...
function serializeThisNodeObjects(node,indentLevel,sourceFile,parent = null,extendNum = 0) {
    return printRecursiveFrom(node,indentLevel+1,sourceFile,parent,extendNum);
}

function printRecursiveFrom(node,indentLevel,sourceFile,parent=null,extendNum = 0){
    ...
    if(ts.isSourceFile(node)){            //305  SourceFile
        ...
        if(node.text == "Infinity"){
            ...
            content = {"type":"NumberLiteral","value":  KeepNDecimal(Number(node.text),6)}
    ...
    else if(ts.isStringLiteral(node)){   //10   StringLiteral-->StringLiteral
    ...
    else if(node.kind === 17 ){  //  17 TemplateTail  
    }
    else if(node.kind === 25 ){  //  25 DotDotDotToken--> 

    }
    else if(node.kind === 27) { //CommaToken
        return ","
    }
    else if(node.kind === 28 ){  //QuestionDotToken

    }
    else if(node.kind ===29){   // LessThanToken
       return '<';
    }
    else if(node.kind === 31){   // 31 GreaterThanToken 
        return '>'
    }
    else if(node.kind === 34 ){  //EqualsEqualsToken
        return "==";
    }
    else if(node.kind === 36 ){  //EqualsEqualsEqualsToken
        return "===";
    }
    else if(node.kind === 37){  //ExclamationEqualsEqualsToken
        return "!=="
    }
    else if(node.kind === 38 ){ // 38 EqualsGreaterThanToken
        return true;
    }
    else if(node.kind === 39 ){ // PlusToken
       return '+';
        
    }else if(node.kind === 40 ){   //MinusToken
       return '-';
    }
    else if(ts.isAsteriskToken(node)){       // 41 asteriskToken  ==> judge the generator
        if(ts.isBinaryExpression(parent)){
            return "*";
        }
        return true;
    }
    else if(node.kind === 42 ){   //42  AsteriskAsteriskToken
        return '**';
    }
     else if(node.kind === 43 ){   //  43  SlashToken
        return '/';
    }
    else if(node.kind === 44 ){   // 44  PercentToken
        return '%';
    }
    else if(node.kind ===45 ){  //45 PlusPlusToken
        return '++';
    }
    else if(node.kind ===46){   //46 MinusMinusToken
        return  '--';
    }
    else if(node.kind === 53){  // 53 Exclamation
        return '!';
    }
    else if(node.kind === 54){  //54 TildeToken
        return '~';
    }
    else if(node.kind === 55){  // 55 AmpersandAmpersandToken
        return '&&';
    }
    else if(node.kind === 56){  // 56 BarBarToken
        return '||';
    }
    else if(node.kind ===57 ){ //57 QuestionToken
        return true;
    }
    else if(node.kind === 60 ){  // 60 QuestionQuestionToken
        return "??";
    }
    else if(node.kind === 63){  //63 EqualsToken
        return "="
    }
    else if(node.kind === 75){  //75 BarBarEqualsToken
        return "||="
    }
    else if(node.kind === 76){  //76 AmpersandAmpersandEqualsToken
        return "&&="
    }
    else if(node.kind === 77){  //77 QuestionQuestionEqualsToken
        return "??="
    }
    else if(ts.isIdentifier(node)){               //79  Identifier-->Identifier
        const name_kind = syntaxKind;   
        const name_escapedText =node.escapedText;
        content={"type":name_kind,"name":name_escapedText,"decorators": []}; 
    }
    else if(ts.isPrivateIdentifier(node)){ // 80 PrivateIdentifier-->TSPrivateIdentifier
       content ={"type":"Identifier"}
       content.name = node.escapedText.substring(1)
       content.decorators = []
    }
    else if(node.kind === 93 ){  //93 ExportKeyword = 93,
        return true
    }
    else if(node.kind === 94) {  //  94 ExtendsKeyword
        //None
    }
    else if(node.kind === 95) {  //  95 FalseKeyword
        content = {"type":"BooleanLiteral"}
        content.value = false;
    }
    else if(node.kind ===102){  //102   InstanceOfKeyword 
        return "instanceof";
    }
    else if(node.kind ===103){  //103   NewKeyword 
        
    }
    else if(node.kind ===104 ){  //104 NullKeyword --> NullLiteral
    ...
    else if (node.kind === 121 )   //121  PrivateKeyword-->
    {   
        return "private"
    }
    else if(node.kind === 122){  //  122   ProtectedKeyword 
        return "protected"
    }
    else if(node.kind === 123)  {         //123 PublicKeyword
    ...
    else if(node.kind === 128){  //  128   AssertsKeyword 
        return true
    }
    else if(node.kind === 124){  //  124   StaticKeyword 
        return "static"
    }
    ...
    else if (ts.isParameter(node)){   //164 Parameter(only)
       if(node.modifiers){
            content = serializeThisNodeObjects(node.modifiers[0],indentLevel,sourceFile);
            content.parameter = serializeThisNodeObjects(node.name,indentLevel,sourceFile);
            content.parameter.typeAnnotation=serializeThisNodeObjects(node.type,indentLevel,sourceFile);
            content.loc = {"start":getRowandColumn(node.getStart(sourceFile),code),"end":getRowandColumn(node.name.end,code)};
        }else if(node.initializer){
            content = {"type":"AssignmentPattern"};
            content.left = serializeThisNodeObjects(node.name,indentLevel,sourceFile);
            content.right = serializeThisNodeObjects(node.initializer,indentLevel,sourceFile);
            content.loc = {"start":getRowandColumn(node.getStart(sourceFile),code),"end":getRowandColumn(node.name.end,code)};
        }else if(node.dotDotDotToken){
            content = {"type":"RestElement"}
            content.argument = serializeThisNodeObjects(node.name,indentLevel,sourceFile)
            if(node.type){
                content.argument.typeAnnotation=serializeThisNodeObjects(node.type,indentLevel,sourceFile);
            }
            content.loc = {"start":getRowandColumn(node.getStart(sourceFile),code),"end":getRowandColumn(node.name.end,code)};
        }else{ 
       ...
       return content;
    ...
    else if(ts.isDecorator(node)){  //  165 Decorator-->Decorator
        ...
        if(ts.isPrivateIdentifier(node.name)){  // special condition: PrivateIdentifier
            ...
            if(node.initializer){
                content.key.value = serializeThisNodeObjects(node.initializer,indentLevel,sourceFile);
            }
            if(node.type){
                content.key.typeAnnotation= serializeThisNodeObjects(node.type,indentLevel,sourceFile);
            }
            content.key.loc = {"start":getRowandColumn(node.name.getStart(sourceFile),code),"end":getRowandColumn(node.name.end,code)}
        }else{
            content.key =serializeThisNodeObjects(node.name,indentLevel,sourceFile);
            content.static= false;
            content.readonly= false;
            ...
            if(node.modifiers){
                for(var i =0 ;i <node.modifiers.length;i++){
                    switch(node.modifiers[i].kind){
                        case 121: content.accessibility = 'private';
                        break;
                        case 122: content.accessibility = 'protected';
                        break;
                        case 123: content.accessibility = 'public';
                        break;
                        case 124: content.static = true;
                        break;
                    }
                }
            }
            if(node.initializer){
                content.value = serializeThisNodeObjects(node.initializer,indentLevel,sourceFile);
            }
            if(node.type){
                content.typeAnnotation= serializeThisNodeObjects(node.type,indentLevel,sourceFile);
            }
            ...
            if(node.decorators){
                for(var i=0; i <node.decorators.length;i++){
                    content.decorators[i] = serializeThisNodeObjects(node.decorators[i],indentLevel,sourceFile);
                }
            } 
            } 
    }
        ...
        if(node.typeParameters){
            ...
            for(var i=0; i<node.typeParameters.length ; i++){
                content.typeParameters.params[i] = serializeThisNodeObjects(node.typeParameters[i],indentLevel,sourceFile,node)
            }
        }
        if(node.questionToken) {content.optional= true;}
    }
    else if(ts.isMethodDeclaration(node)){   //169  MethodDeclaration -->  MethodDefinition
        if(parent && ts.isObjectLiteralExpression(parent)){   // 205 Attention！！！！！！！！！！
        ...
        // common feature
        content.key = serializeThisNodeObjects(node.name,indentLevel,sourceFile)
        if(ts.isComputedPropertyName(node.name)) {content.computed=true}
        content.value.function.params=[]
        for(var i =0; i<node.parameters.length ;i++){
            content.value.function.params[i] = serializeThisNodeObjects(node.parameters[i],indentLevel,sourceFile);
        }
        if(node.typeParameters){
            content.value.function.typeParameters = {"type":"TSTypeParameterDeclaration"}
            content.value.function.typeParameters.params = []
            for(var i =0; i<node.typeParameters.length ;i++){
                content.value.function.typeParameters.params[i] = serializeThisNodeObjects(node.typeParameters[i],indentLevel,sourceFile);
            }
        }
        if(node.type){
            content.value.function.returnType = serializeThisNodeObjects(node.type,indentLevel,sourceFile);
        }
        if(node.body){
            content.value.function.body = serializeThisNodeObjects(node.body,indentLevel,sourceFile);
        }
        content.value.function.generator = (node.asteriskToken && node.asteriskToken.kind === 41 )? true : false;
        if(node.modifiers){
            for(var i =0; i< node.modifiers.length ; i++){
                switch (node.modifiers[i].kind) {
                    case 131:
    ...
    else if (ts.isConstructorDeclaration(node)){  //171  ConstructorDeclaration-->
         content = {"type":"MethodDefinition","key":{"type":"Identifier","name":"constructor","decorators":[]}};
    ...
    else if(ts.isGetAccessor(node)){  //172 GetAccessor-->
        if(parent && parent.kind ===205 ){  //ObjectExpression
            content = {"type":"Property"}
        ...
        content.key = serializeThisNodeObjects(node.name,indentLevel,sourceFile);
        content.kind = "get"
        content.value = {"type":"FunctionExpression","function":{"type":"ScriptFunction","id":null,"generator":false,"async":false,"expression":false,"params":[]}}
        for(var i=0; i<node.parameters.length;i++){
            content.value.function.params[i] = serializeThisNodeObjects(node.parameters[i],indentLevel,sourceFile)
        }
        if(node.body){
        ...
        if(node.name.kind === 162 )  { content.computed = true } //  162  ComputedPropertyName--> []  computed: true 
        content.overloads = []
        content.decorators =[]
        if(node.decorators){
            for(var i=0; i <node.decorators.length;i++){
    ...
    else if(ts.isSetAccessor(node)){  //173 SetAccessor-->
        if(parent && parent.kind ===205 ){  //ObjectExpression
            content = {"type":"Property"}
            content.method =false;
            content.shorthand =false;
        ...
        if(node.type){
            content.returnType = serializeThisNodeObjects(node.type,indentLevel,sourceFile)
        }
        if(node.typeParameters){
            content.typeParameters = {"type":"TSTypeParameterDeclaration"}
            content.typeParameters.params = []
            for(var i=0 ; i <node.typeParameters.length;i++){
                content.typeParameters.params[i] = serializeThisNodeObjects(node.typeParameters[i],indentLevel,sourceFile)
            }    
        }
    }
        ...
        if(node.typeArguments){
            ...
            for(var i=0; i<node.typeArguments.length ; i++){
                content.typeParameters.params[i] = serializeThisNodeObjects(node.typeArguments[i],indentLevel,sourceFile,node)
            }
    ...
    else if(ts.isLiteralTypeNode(node)){  // 196   LiteralType-->TSLiteralType
        ...
        if(node.elements.length>0){
            for(var i=0; i <node.elements.length; i++ ){
                content.properties[i]=serializeThisNodeObjects(node.elements[i],indentLevel,sourceFile);
            }
        }
    }
    ...
    else if(ts.isBindingElement(node)){      //203 BindingElement-->Property
        if(node.dotDotDotToken && node.dotDotDotToken.kind === 25)  {  // DotDotDotToken
            content = {"type":"RestElement"};
            content.argument = serializeThisNodeObjects(node.name,indentLevel,sourceFile);
        }else if(parent &&  ts.isArrayBindingPattern(parent) && node.initializer){  //202
            content= {"type":"AssignmentPattern"};
            content.left =serializeThisNodeObjects(node.name,indentLevel,sourceFile)
            content.right =serializeThisNodeObjects(node.initializer,indentLevel,sourceFile)
        }
            ...
            if(node.propertyName && node.initializer){
                content.key= serializeThisNodeObjects(node.propertyName,indentLevel,sourceFile)
                content.value= {"type":"AssignmentPattern"};
                content.value.left =serializeThisNodeObjects(node.name,indentLevel,sourceFile)
                content.value.right =serializeThisNodeObjects(node.initializer,indentLevel,sourceFile)
                if(node.propertyName.kind === 162 )  { content.computed = true } //  162  ComputedPropertyName--> []  computed: true 
            }else if(node.propertyName){
                content.key= serializeThisNodeObjects(node.propertyName,indentLevel,sourceFile)
                content.value= serializeThisNodeObjects(node.name,indentLevel,sourceFile)
                if(node.propertyName.kind === 162 )  { content.computed = true } //  162  ComputedPropertyName--> []  computed: true
            }else if(node.initializer){
                content.key= serializeThisNodeObjects(node.name,indentLevel,sourceFile)
                content.value = serializeThisNodeObjects(node.initializer,indentLevel,sourceFile)
        ...
        if(node.elements.length>0){
            for(var i=0; i <node.elements.length; i++ ){
                content.elements[i]=serializeThisNodeObjects(node.elements[i],indentLevel,sourceFile);
            }
        }
    ...
    else if(ts.isPropertyAccessExpression(node)){   //  206  PropertyAccessExpression  --> MemberExpression
        ...
        // }else{
            ...
            content.computed = false;
            content.optional = false; if(node.questionDotToken) {content.optional = true;}
        //}
    }
        ...
        // }else{
            ...
            content.optional = false;
            if(node.questionDotToken) {content.optional = true;}
        //}
    }
    else if(ts.isCallExpression(node)){           //208 CallExpression -->  CallExpression
        content={"type":syntaxKind};
        content.callee= serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
        content.arguments = [];
        if(node.arguments.length > 0){
            for(var i=0; i < node.arguments.length; i++){
                content.arguments[i]=serializeThisNodeObjects(node.arguments[i],indentLevel,sourceFile)
            }
        }
        if(node.typeArguments){
            content.typeParameters = {"type":"TSTypeParameterInstantiation"}
            content.typeParameters.params = []
            for(var i=0 ; i<node.typeArguments.length ;i++){
                content.typeParameters.params[i] = serializeThisNodeObjects(node.typeArguments[i],indentLevel,sourceFile)
            }
        }
        content.optional = false;
        if(node.questionDotToken) {content.optional = true;}
    ...
    else if(ts.isNewExpression(node)){  // 209 NewExpression-->NewExpression
        content = {"type":syntaxKind};
        content.callee = serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
        if(node.arguments){
            content.arguments = [];
    ...
    else if(ts.isTaggedTemplateExpression(node)){  // 210 TaggedTemplateExpression-->TaggedTemplateExpression
        content = {"type":syntaxKind}
    ...
    else if(ts.isParenthesizedExpression(node)) { // 212  ParenthesizedExpression -->
        return serializeThisNodeObjects(node.expression,indentLevel,sourceFile,node);
    }
    else if(ts.isFunctionExpression(node)) {       //213  FunctionExpression --> FunctionExpression
        content={"type":"FunctionExpression","function":{"type":"ScriptFunction","id":null,"generator":false,"async":false,"expression":false}};
        ...
        content.function.generator = (node.asteriskToken && node.asteriskToken.kind === 41 )? true : false;
        content.function.params = []
        for(var i=0; i < node.parameters.length; i++){
            content.function.params[i] = serializeThisNodeObjects(node.parameters[i],indentLevel,sourceFile);
        }
        content.function.body = serializeThisNodeObjects(node.body,indentLevel,sourceFile);
        content.function.loc ={"start":getRowandColumn(node.getStart(sourceFile),code),"end":getRowandColumn(node.end,code)};
        content.overloads=[];
        content.decorators=[];
        if(node.decorators){
            for(var i=0; i <node.decorators.length;i++){
        ...
        if(node.name){
            content.function.id = serializeThisNodeObjects(node.name,indentLevel,sourceFile);
        }
        if(node.typeParameters){
            content.function.typeParameters = {"type":"TSTypeParameterDeclaration"}
            content.function.typeParameters.params = []
            for(var i=0 ; i <node.typeParameters.length;i++){
                content.function.typeParameters.params[i] = serializeThisNodeObjects(node.typeParameters[i],indentLevel,sourceFile)
            }    
        }
    }
        ...
        if(node.modifiers){
            for(var i =0; i <node.modifiers.length;i++){
                if(node.modifiers[i].kind === 131){content.function.async = true;}
            }
        }
        content.function.params = []
        for(var i=0;i< node.parameters.length;i++){
        ...
        if(node.body.kind != 235 ) {content.function.expression = true;}   // if it is { } block
        if(node.type) content.function.returnType = serializeThisNodeObjects(node.type,indentLevel,sourceFile);
        if(parent && ts.isParenthesizedExpression(parent)){
            var  start = parent.getStart(sourceFile);
            var  end = parent.getEnd();
        }
    ...
    else if(ts.isAwaitExpression(node)){     // 218 AwaitExpression-->AwaitExpression
        content = {"type":syntaxKind}
        content.argument = serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
    }
    else if(ts.isPrefixUnaryExpression(node)){ //219 PrefixUnaryExpression --> UnaryExpression
        ...
        if(tmpOperator === 45 || tmpOperator === 46 ){
            ...
            switch(node.operator){
                ...
                break;
                case 40: content.operator = '-'
                break;
                case 53: content.operator = '!'
                break;
                case 54: content.operator = '~'
                break;
            }
        }
        if(parent && ts.isParenthesizedExpression(parent)){
    ...
    else if(ts.isBinaryExpression(node)) {// 221 BinaryExpression --> BinaryExpression
        operator = serializeThisNodeObjects(node.operatorToken,indentLevel,sourceFile,node);
            ...
            function GetSequenceExoression(tmpnode){
                if(tmpnode.operatorToken ){
                    if(tmpnode.operatorToken.kind === 27){
                        ...
                        tempexpressions[num++] = serializeThisNodeObjects(tmpnode,indentLevel,sourceFile)
                        return;
                    }
                }else{
                    tempexpressions[num++] = serializeThisNodeObjects(tmpnode,indentLevel,sourceFile)
                ...
                if(tmpnode.right){
                    tempexpressions[num++] = serializeThisNodeObjects(tmpnode.right,indentLevel,sourceFile)
                }
            }
        ...
        content.operator = serializeThisNodeObjects(node.operatorToken,indentLevel,sourceFile,node);
        content.left =serializeThisNodeObjects(node.left,indentLevel,sourceFile)
        content.right = serializeThisNodeObjects(node.right,indentLevel,sourceFile)
    }
    else if(ts.isConditionalExpression(node)) {  //222 ConditionalExpression-->ConditionalExpression
        content = {"type":syntaxKind}
        content.test = serializeThisNodeObjects(node.condition,indentLevel,sourceFile)
        content.consequent = serializeThisNodeObjects(node.whenTrue,indentLevel,sourceFile)
        ...
        content.quasis= serializeThisNodeObjects(node.head,indentLevel,sourceFile)
    }
    else if(ts.isYieldExpression(node)){   //   224 YieldExpression  --> YieldExpression
        content={"type":syntaxKind}
        content.delegate =( node.asteriskToken && node.asteriskToken.kind === 41 )? true : false;
        content.argument = node.expression ? serializeThisNodeObjects(node.expression,indentLevel,sourceFile) : null;
    }
    else if(ts.isSpreadElement(node)){    //  225 SpreadElement-->SpreadElement
        content = {"type":syntaxKind};
        content.argument = serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
    }
    else if(ts.isClassExpression(node)){  // 226 ClassExpression --> ClassExpression
        content = {"type":syntaxKind};
        content.definition = {"id": node.name ? serializeThisNodeObjects(node.name,indentLevel,sourceFile) : null};
        content.definition.superClass =node.heritageClauses ? serializeThisNodeObjects(node.heritageClauses[0].types[0].expression,indentLevel,sourceFile) : null ;
        if(node.heritageClauses && node.heritageClauses[0].types[0].typeArguments){
            content.definition.superTypeParameters  = {"type":"TSTypeParameterInstantiation"}
            content.definition.superTypeParameters.params = []
            for(var i=0;  i< node.heritageClauses[0].types[0].typeArguments.length;i++){
                content.definition.superTypeParameters.params[i] =node.heritageClauses ? serializeThisNodeObjects(node.heritageClauses[0].types[0].typeArguments[i],indentLevel,sourceFile) : null ;
            }
        }
        ...
        for(var i =0 ; i <node.members.length;i++){
            content.definition.body[i] = serializeThisNodeObjects(node.members[i],indentLevel,sourceFile);
        }
        content.definition.indexSignatures = []
        if(node.typeParameters){
            content.definition.typeParameters = {"type":"TSTypeParameterDeclaration"}
            content.definition.typeParameters.params = []
            for(var i=0;  i<node.typeParameters.length;i++){
                content.definition.typeParameters.params[i] = serializeThisNodeObjects(node.typeParameters[i],indentLevel,sourceFile)
            }
        }
    }
    else if(ts.isOmittedExpression(node)){  // 227 OmittedExpression --> OmittedExpression
        content={"type":syntaxKind}
    }
    else if (ts.isExpressionWithTypeArguments(node)){   //228 ExpressionWithTypeArguments-->
        return serializeThisNodeObjects(node.expression,indentLevel,sourceFile); 
    }
    ...
    else if(ts.isMetaProperty(node)){  // 231 MetaProperty-->MetaProperty
        content = {"type":syntaxKind}
        content.kind = "new.Target"
    }
    else if(ts.isTemplateSpan(node)){  // 233 TemplateSpan-->
        return serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
    }
    ...
    else if(ts.isVariableStatement(node)){        //237
        return serializeChildObjects(node,indentLevel,sourceFile);
    }
    else if(ts.isExpressionStatement(node)){  //238  ExpressionStatement-->ExpressionStatement
        content ={"type":syntaxKind};
        content.expression = serializeChildObjects(node,indentLevel,sourceFile)[0];
    }
    else if(ts.isIfStatement(node)){  // 239 IfStatement --> IfStatement
        content = {"type":syntaxKind};
        content.test =serializeThisNodeObjects(node.expression,indentLevel,sourceFile);
        content.consequent = serializeThisNodeObjects(node.thenStatement,indentLevel,sourceFile);
    ...
    else if(ts.isWhileStatement(node)){  //241 WhileStatement -->WhileStatement
        content = {"type":syntaxKind}
        content.test = serializeThisNodeObjects(node.expression, indentLevel, sourceFile)
        content.body = serializeThisNodeObjects(node.statement, indentLevel, sourceFile)
    }
    ...
    else if(ts.isForInStatement(node)){   //243  ForInStatement -->  ForInStatement
        content = {"type":syntaxKind}
        content.left = serializeThisNodeObjects(node.initializer, indentLevel, sourceFile)
        content.right =  serializeThisNodeObjects(node.expression , indentLevel , sourceFile)
        content.body = serializeThisNodeObjects(node.statement,indentLevel,sourceFile)
    }
    else if(ts.isForOfStatement(node)){   //244  ForOfStatement -->  ForOfStatement
        content = {"type":syntaxKind}
        content.await = false ;
        content.left = serializeThisNodeObjects(node.initializer, indentLevel, sourceFile)
        content.right =  serializeThisNodeObjects(node.expression , indentLevel , sourceFile)
    ...
    else if(ts.isContinueStatement(node)) {  //245 ContinueStatement-->ContinueStatement
        content = {"type":syntaxKind};
        content.label = node.label ? serializeThisNodeObjects(node.label,indentLevel,sourceFile): null;
    }
    else if(ts.isBreakStatement(node)){  //246 BreakStatement-->BreakStatement
        content = {"type":syntaxKind};
        content.label = node.label ? serializeThisNodeObjects(node.label,indentLevel,sourceFile): null;
    }
    else if(ts.isReturnStatement(node)) {  //247 ReturnStatement --> ReturnStatement
        content={"type":syntaxKind}
        content.argument= node.expression ? serializeThisNodeObjects(node.expression,indentLevel,sourceFile):null;
    }
    else if(ts.isSwitchStatement(node)){  //249 SwitchStatement-->SwitchStatement
        content = {"type":syntaxKind};
        content.discriminant = serializeThisNodeObjects(node.expression,indentLevel,sourceFile)
        content.cases = []
        for (var i=0 ; i< node.caseBlock.clauses.length;i++){
            content.cases[i] = serializeThisNodeObjects(node.caseBlock.clauses[i],indentLevel,sourceFile)
        }
    }
    ...
    else if(ts.isThrowStatement(node)){  //251 ThrowStatement --> ThrowStatement
        content = {"type":syntaxKind}
        content.argument = serializeThisNodeObjects(node.expression,indentLevel,sourceFile)
    }
    else if(ts.isTryStatement(node)){   //252 TryStatement --> TryStatement
        content={"type":syntaxKind}
        content.block = serializeThisNodeObjects(node.tryBlock,indentLevel,sourceFile)
        if(node.catchClause){
            content.handler = serializeThisNodeObjects(node.catchClause,indentLevel,sourceFile)
        }else{
        ...
        if(node.type){
            content.id.typeAnnotation = serializeThisNodeObjects(node.type,indentLevel,sourceFile);
        }
    }
        ...
        if(parent && ts.isVariableStatement(parent) && variableStatementHasSemicolon(end,code) > -1 ){
            end = parent.end;        //！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
        }//！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！
    }
    else if(ts.isFunctionDeclaration(node)){   //256   FunctionDeclaration-->FunctionDeclaration
        content={"type":syntaxKind};
    ...
    else if(ts.isClassDeclaration(node)){  //257  ClassDeclaration --> ClassDeclaration
        content = {"type":syntaxKind,"definition":{"id":serializeThisNodeObjects(node.name,indentLevel,sourceFile)}};
        content.definition.implements = [];
        content.definition.superClass = null;
        if(node.heritageClauses){  // superClass is not null
            for(var i =0; i <node.heritageClauses.length; i++){
                if(node.heritageClauses[i].token === 94) {  //  94 ExtendsKeyword
                    content.definition.superClass = serializeThisNodeObjects(node.heritageClauses[i],indentLevel,sourceFile)[0];
                }else if(node.heritageClauses[i].token === 117 ){  ///implements
                    content.definition.implements[i] = serializeThisNodeObjects(node.heritageClauses[i],indentLevel,sourceFile);
                }
            }
        }
        content.definition.indexSignatures=[];
        // variables and constructer are in the members.  ！！！！！！！Attention！！！！！！！！！
        if(node.members.length>0){ // constructor exists
            content.definition.body = []
            for(var i=0,j=0,k=0; i<node.members.length; i++){
                var memberKind= node.members[i].kind;
                if( memberKind === 171 ){
                    content.definition.constructor=serializeThisNodeObjects(node.members[i],indentLevel,sourceFile);
                }else if(memberKind === 176){
                    content.definition.indexSignatures[k]=serializeThisNodeObjects(node.members[i],indentLevel,sourceFile);
                    k++;
                }else{
                    content.definition.body[j] = serializeThisNodeObjects(node.members[i],indentLevel,sourceFile);
                    j++;
                }
            }     
        }
        ...
        if(node.heritageClauses){
            for(var i=0; i< node.heritageClauses[0].types.length;i++){
                content.extends[i]=serializeThisNodeObjects(node.heritageClauses[0],indentLevel,sourceFile,node,i) //extendNum
            }
        }
        if(node.typeParameters){
            content.typeParameters = {"type":"TSTypeParameterDeclaration"};
    ...
    else if(ts.isCaseBlock(node)){  //263 CaseBlock  --> 
        ...
        content.consequent = []
        for(var i=0; i<node.statements.length;i++){
            content.consequent[i] = serializeThisNodeObjects(node.statements[i],indentLevel,sourceFile);
        }
    }
    ...
    else if(ts.isHeritageClause(node)){   //  291  HeritageClause-->TSInterfaceHeritage
        if(node.token === 117 ){  // 117 ImplementsKeyword
            content = {"type":"TSClassImplements"}
            content.expression = serializeThisNodeObjects(node.types[extendNum],indentLevel,sourceFile);
            if(node.types[extendNum].typeArguments){
                content.typeParameters = {"type":"TSTypeParameterInstantiation"}
                content.typeParameters.params=[]
                for(var i=0; i<node.types[extendNum].typeArguments.length; i++){
                    content.typeParameters.params[i] = serializeThisNodeObjects(node.types[extendNum].typeArguments[i],indentLevel,sourceFile,node)
                }
            }
        }else if(node.token === 94 && parent !== null && ts.isInterfaceDeclaration(parent)){   //ExtendsKeyword 94   258   ==>according to 'arrayLiterals2ES6.ts'
            content = {"type":"TSInterfaceHeritage"};
            content.expression = {"type":"TSTypeReference"}
            content.expression.typeName = serializeThisNodeObjects(node.types[extendNum].expression,indentLevel,sourceFile)
            if(node.types[extendNum].typeArguments){
                content.expression.typeParameters = {"type":"TSTypeParameterInstantiation"}
                content.expression.typeParameters.params=[]
                for(var i=0; i<node.types[extendNum].typeArguments.length; i++){
                    content.expression.typeParameters.params[i] = serializeThisNodeObjects(node.types[extendNum].typeArguments[i],indentLevel,sourceFile,node)
                }
            }
        }else{
    ...
    else if(ts.isCatchClause(node)){  //292 CatchClause -> CatchClause
        content = {"type": syntaxKind}
        content.body= serializeThisNodeObjects(node.block,indentLevel,sourceFile)
        content.param = null
        if(node.variableDeclaration){
            const child_node = node.variableDeclaration.name;
            content.param = serializeThisNodeObjects(child_node,indentLevel,sourceFile);
        }
    }
    ...
    if(ts.isSourceFile(node)) {start_rowAndcol=getRowandColumn(0,code);}
      ...
      if (!cachedArgs[l_str] || !cachedArgs[l_str][r_str]) {
        ...
        for (let i = 0; i < allKeys.length; i++) {
          ...
          if (left[key] !== right[key]) {
            ...
            if(left.type == "ChainExpression" || right.type == "ChainExpression"|| left.kind =="constructor" || right.type == "EmptyStatement"|| right.type == "EmptyStatement"){
                continue;
            }                            
            if( left.type=="TSInterfaceBody" || left.name =="constructor"  || left.type =="FunctionExpression"|| 
            left.type=="ClassProperty" || left.type =="ScriptFunction" || left.type =="ClassDeclaration" || left.type == "TSTypeParameterDeclaration"){
                    
                    if( left.type =="ClassDeclaration"){
                        delete left.definition.loc;
                        if(right.hasOwnProperty("definition") && right.definition.hasOwnProperty("constructor") && JSON.stringify(right.definition.constructor)==='{}'){
                            delete left.definition.constructor;
                        }
                    }else if(left.type == "FunctionExpression"){  //有两个FunctionExpression，一个是Node.kind匹配的，一个是自己添加在某个Node的。下面删除需要在自己添加的删除掉
                        if(!right.loc){ //如果是我添加的，则位置信息不需要比较
                            delete left.loc;  // the location of {}. need to delete
                            delete left.function.loc;
                        }
                    }else if(left.type == "ScriptFunction"){  //如果funtion存在修饰符时，比如async，那么ScriptFunction的start位置会报错
                            delete left.loc;  
                            delete right.loc;
                    }
                    else if(left.type == "TSTypeParameterDeclaration"){  //es2panda的loc是自己添加的。 如 <T> ,而 tsc只考虑 T 
                        delete left.loc;  
                    }
                    else if(left.type == "ClassProperty"){      //它的位置只包含了name的loc 
                        delete left.loc.end; 
