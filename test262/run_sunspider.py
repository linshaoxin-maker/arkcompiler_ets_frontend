...
class ArkProgram():
    ...
    def gen_abc(self):
        ...
        if file_name in self.module_list and self.ark_frontend == ARK_FRONTEND_LIST[1]:
            dependencies = collect_module_dependencies(js_file,
                os.path.join(TEST_ES2021_DIR, "language/module-code"), [])
        ...
        if self.ark_frontend == ARK_FRONTEND_LIST[0]:
            ...
            if merge_abc_mode != "0":
                ...
                cmd_args = [frontend_tool, '--opt-level=' + str(self.opt_level),
                            '--thread=' + str(self.es2abc_thread_count),
                            '--output', out_file, js_file]
            if file_name in self.module_list:
