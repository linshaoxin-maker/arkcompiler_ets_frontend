...
export function compileYieldExpression(compiler: Compiler, expr: ts.YieldExpression) {
    if (!(compiler.getFuncBuilder() instanceof GeneratorFunctionBuilder || compiler.getFuncBuilder() instanceof AsyncGeneratorFunctionBuilder)) { //new add
...
function genYieldExpr(compiler: Compiler, expr: ts.YieldExpression) {
    ...
    let funcBuilder = <GeneratorFunctionBuilder | AsyncGeneratorFunctionBuilder> compiler.getFuncBuilder(); //new add
    if (expr.expression) {
...
function genYieldStarExpr(compiler: Compiler, expr: ts.YieldExpression) {
    let funcBuilder = <GeneratorFunctionBuilder>compiler.getFuncBuilder();//hyq
    if (!expr.expression) {
