    ...
    private storeFuncObj2LexEnvIfNeeded() {
        ...
        if ((<ts.FunctionLikeDeclaration>rootNode).name) {
            ...
            if (v.scope == functionScope) {
                this.pandaGen.loadAccumulator(NodeKind.Invalid, getVregisterCache(this.pandaGen, CacheList.FUNC));
                this.pandaGen.storeAccToLexEnv(NodeKind.Invalid, v.scope, v.level, v.v, true);
            }
        }
    }
