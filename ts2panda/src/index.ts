    ...
    let emitResult = program.emit(
        ...
        undefined,
        {
            ...
            after: [
                ...
                (ctx: ts.TransformationContext) => {
                    return (node: ts.SourceFile) => {
                        makeAutoGeneratedNodeName(node);
...
function makeAutoGeneratedNodeName(node: ts.Node) {
        ...
        switch (childNode.kind) {
            case ts.SyntaxKind.Identifier: {
                ...
                if (ts.isGeneratedIdentifier(childNode)) {
                    // @ts-ignore
                    let generatedVarId = (<ts.Identifier>childNode).autoGenerateId;
                    if (generatedVarId < 26) {  // _a ~ _z
                        // @ts-ignore
                        (<ts.Identifier>childNode).escapedText = "#" + String.fromCharCode(97 /* a */ + generatedVarId);
                    } else {
                        // @ts-ignore
                        (<ts.Identifier>childNode).escapedText = "#" + (generatedVarId - 26);
                    }
                }
                break;
            }
        }
        makeAutoGeneratedNodeName(childNode);
    });
}
