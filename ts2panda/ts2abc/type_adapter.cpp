...
{
    return idx < func->params.size() && func->ins[idx].opcode == panda::pandasm::Opcode::MOV;
}
                                        ...
                                        const std::unordered_map<int32_t, int32_t> &vreg_type_map) const
{
    ...
    for (size_t i = 0; i < func->ins.size(); i++) {
        ...
        if (!maybe_arg && insn.opcode != panda::pandasm::Opcode::STA) {
            continue;
        }
        if (maybe_arg) {
        ...
        ASSERT(insn.opcode == panda::pandasm::Opcode::STA);
...
{
    if (insn.opcode != panda::pandasm::Opcode::TRYLDGLOBALBYNAME) {
        return;
    }
