...
def get_commit_records(repo_name, commit_start_time, commit_end_time):
    ...
    if not has_commit_log:
        ...
        failed_message = f'this repo has no commit record from {commit_start_time} to{commit_end_time.strftime("%Y-%m-%d %H:%M:%S")}'
...
def run():
    ...
    if arguments.start_time is not None:
        ...
        start_time = datetime.combine(time_str, datetime.time.min)
