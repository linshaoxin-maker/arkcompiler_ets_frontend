...
def get_download_url(download_file_path):
    dayu200_url = None
    sdk_url = None
    with open(download_file_path, 'r') as file:
        for line in file:
            ...
            if len(parts) == 2:
                ...
                if 'sdk' in url:
                    sdk_url = url
    return sdk_url, dayu200_url


def send_email():
    with open(r"send_email/config.yaml", 'r') as f:
    ...
    commit_log = data["commit_log_report_file"]
    ...
    download_file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '../download/download_url.txt')
    if os.path.exists(download_file_path):
        sdk_url, dayu200_url = get_download_url(download_file_path)
    ...
    html = add_content(html, commit_log, "commit_log")
    msg.attach(MIMEText(html, 'html', 'utf-8'))
