...
export class TypeUtils {
  public static createNewSourceFile(oldAst: SourceFile): SourceFile {
    ...
    // const fileSuffix = oldAst.fileName.endsWith('.ts') ? '.ts' : '.js';
    const fileSuffix: string = '.ts';
    const targetName: string = '__tmp' + Math.random() + fileSuffix;
  ...
  public static createChecker(ast: SourceFile): TypeChecker {
    ...
    const customHost: CompilerHost = {
      ...
      // optional
      getDefaultLibLocation: host.getDefaultLibLocation,
      ...
      getEnvironmentVariable: host.getEnvironmentVariable,
