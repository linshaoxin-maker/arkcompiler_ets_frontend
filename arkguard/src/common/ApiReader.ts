...
import {parseCommonProject, parseOhSdk, parseThirdPartyLibs} from './ApiExtractor';
import {ListUtil} from '../utils/ListUtil';
import type {IOptions} from '../configs/IOptions';
import es5Info from '../configs/preset/es5_reserved_properties.json';
...
export function initPlugin(sdkDir: string, outputDir: string): void {
  ...
  if (!ohSdkPath) {
    console.error('openHarmony sdk path is not found.');
  }

  const apiVersions: string[] = [''];
  // todo：新版sdk文件夹目录布局发生了变化，从 ets/版本 -> 版本/ets。待定

  apiVersions.forEach((versionString) => {
    parseOhSdk(ohSdkPath, versionString, true, outputDir);
  });
}
...
export function readOhReservedProperties(apiSavedPath: string): string[] {
  ...
  // 2. read language reserved property
  reservedProperties = [...reservedProperties, ...es5Info];
...
export function readProjectProperties(projectPaths: string[], customProfiles: IOptions): void {
  ...
  for (const projectPath of projectPaths) {
    ...
    const projProperties: string[] = parseCommonProject(projectPath);
...
function readThirdPartyLibProperties(projectPath: string): string[] {
  ...
  if (!fileNames.includes('node_modules')) {
    return undefined;
  }

  const filePath: string = path.join(projectPath, 'node_modules');
  if (!fs.lstatSync(filePath).isDirectory()) {
    return undefined;
  }

  const properties: string[] = parseThirdPartyLibs(filePath);
  reservedProperties = [...reservedProperties, ...properties];
  const propertySet: Set<string> = new Set<string>(reservedProperties);
