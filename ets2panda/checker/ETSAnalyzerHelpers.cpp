    ...
    if (scriptFunc->IsAsyncFunc()) {
        ...
        if (scriptFunc->ReturnTypeAnnotation() == nullptr) {
            if (scriptFunc->IsAsyncImplFunc()) {
                SetAsyncImplFuncReturnType(checker, scriptFunc);
            }
...
void SetAsyncImplFuncReturnType(ETSChecker *checker, ir::ScriptFunction *scriptFunc)
{
...
void HandleAsyncMethodCheck(ETSChecker *checker, ir::MethodDefinition *node)
{
    ...
    if (node->Function()->IsOverload()) {
        auto *overloadedImplMethod = node->Overloaded()->AsyncPairMethod();
        implMethod->Function()->Id()->SetVariable(overloadedImplMethod->Function()->Id()->Variable());
        overloadedImplMethod->AddOverload(implMethod);
    } else {
