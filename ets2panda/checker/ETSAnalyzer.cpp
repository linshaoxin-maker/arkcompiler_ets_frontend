...
void CheckObjectExpressionType(ir::Expression *st_argument, checker::Type *func_return_type)
{
...
void CheckMemberExpressionType(ir::Expression *st_argument, ETSChecker *checker, checker::Type *func_return_type)
{
...
void CheckSignatureFlag(ETSChecker *checker, ir::ScriptFunction *containing_func, checker::Type *&func_return_type,
                        ir::Expression *st_argument)
{
    //  First (or single) return statement in the function:
    func_return_type =
        st_argument == nullptr ? checker->GlobalBuiltinVoidType() : st_argument->Check(checker);
...
void CheckNoSignatureFlag(ETSChecker *checker, ir::ScriptFunction *containing_func, checker::Type *&func_return_type,
...
checker::Type *ETSAnalyzer::Check(ir::ReturnStatement *st) const
{
    ...
    if (auto *const return_type_annotation = containing_func->ReturnTypeAnnotation();
        ...
        if (st->argument_ == nullptr) {
            ...
            CheckObjectExpressionType(st->argument_, func_return_type);
            CheckMemberExpressionType(st->argument_, checker, func_return_type);
        ...
        if (containing_func->Signature()->HasSignatureFlag(checker::SignatureFlags::NEED_RETURN_TYPE)) {
            CheckSignatureFlag(checker, containing_func, func_return_type, st->argument_);
        } else {
            //  All subsequent return statements:
            CheckNoSignatureFlag(checker, containing_func, func_return_type, st, st->argument_);
        }
    }
