...
bool ETSChecker::VerifySignatureVisibility(Signature *signature, const lexer::SourcePosition &pos)
{
    if (signature == nullptr) {
        LogTypeError({"Signature is not visible here."}, pos);
        return false;
    }
...
{
    if (!VerifySignatureVisibility(signature, pos)) {
        return;
    }
