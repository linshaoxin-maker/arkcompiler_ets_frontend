...
{
    auto *identNode = AllocNode<ir::Identifier>(util::StringView("FunctionalInterface"), Allocator());
...
ir::MethodDefinition *ETSChecker::CreateInvokeFunction(ir::ETSFunctionType *funcType)
{
    ...
    auto *const typeAnnotation = funcType->ReturnType()->Clone(Allocator(), nullptr);
    typeAnnotation->SetTsType(funcType->ReturnType()->TsType());

    ir::ModifierFlags flags = ir::ModifierFlags::ABSTRACT | ir::ModifierFlags::PUBLIC;
    auto *func =
        AllocNode<ir::ScriptFunction>(ir::FunctionSignature(nullptr, std::move(params), typeAnnotation), nullptr,
                                      ir::ScriptFunction::ScriptFunctionData {ir::ScriptFunctionFlags::METHOD, flags});

    func->SetScope(functionScope);
    functionScope->BindNode(func);
    funcParamScope->BindNode(func);

    auto *identNode = AllocNode<ir::Identifier>(util::StringView("invoke"), Allocator());
    func->SetIdent(identNode);

    auto *funcExpr = AllocNode<ir::FunctionExpression>(func);

    auto *identClone = identNode->Clone(Allocator(), nullptr);
    auto *method = AllocNode<ir::MethodDefinition>(ir::MethodDefinitionKind::METHOD, identClone, funcExpr, flags,
                                                   Allocator(), false);
                                                             ...
                                                             ir::MethodDefinition *proxyMethod, bool isStatic)
{
    ...
    for (size_t i = 0; i < counter; i++) {
        ...
        auto *param = AllocNode<ir::Identifier>(classProp->Key()->AsIdentifier()->Name(), Allocator());
    ...
    for (auto const *const it : invokeFunc->Params()) {
        ...
        auto *const paramIdent = AllocNode<ir::Identifier>(param->Ident()->Name(), Allocator());
...
void ETSChecker::ResolveLambdaObjectInvoke(ir::ClassDefinition *lambdaObject, Signature *signatureRef)
{
    ...
    for (auto *it : signatureRef->Params()) {
        ...
        auto *paramIdent = AllocNode<ir::Identifier>(it->Name(), Allocator());
        auto *param = AllocNode<ir::ETSParameterExpression>(paramIdent, nullptr);
        auto [_, var] = VarBinder()->AddParamDecl(param);
        (void)_;
...
ir::Statement *ETSChecker::ResolveLambdaObjectInvokeFuncBody(ir::ClassDefinition *lambdaObject, Signature *signatureRef)
{
    ...
    for (size_t idx = 0; idx != signatureRef->Params().size(); idx++) {
        auto *paramIdent = AllocNode<ir::Identifier>(signatureRef->Params()[idx]->Name(), Allocator());
                                               ...
                                               ir::AstNode *body)
{
    ...
    auto *scope = VarBinder()->Allocator()->New<varbinder::FunctionScope>(Allocator(), paramScope);
    auto *func = AllocNode<ir::ScriptFunction>(ir::FunctionSignature(nullptr, std::move(params), returnType), body,
                                               ir::ScriptFunction::ScriptFunctionData {flags, modifiers});
    func->SetScope(scope);
