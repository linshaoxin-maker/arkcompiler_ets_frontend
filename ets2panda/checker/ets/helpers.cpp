...
checker::Type *ETSChecker::CheckArrayElements(ir::ArrayExpression *init)
{
    ...
    ArenaVector<Type *> elementTypes(Allocator()->Adapter());
    ArenaMap<TypeFlag, Type *> elementTypesMap(Allocator()->Adapter());
    Type *type;
    for (auto &e : elements) {
        auto eType = e->Check(this);
        elementTypesMap[static_cast<TypeFlag>((eType->TypeFlags() & TypeFlag::ETS_TYPE))] = e->Check(this);
        // Enum type needs to be handled here after Enum type is implemented well.
    }
    if (elementTypesMap.empty()) {
        type = GlobalETSObjectType();
    } else {
        if (std::all_of(elementTypesMap.begin(), elementTypesMap.end(),
                        [](auto &it) { return it.second->HasTypeFlag(TypeFlag::ETS_NUMERIC); })) {
            type = GlobalDoubleType();
        } else {
            for (auto &it : elementTypesMap) {
                elementTypes.emplace_back(it.second);
            }
            type = CreateETSUnionType(std::move(elementTypes));
        }
    }
    return Allocator()->New<ETSArrayType>(type);
}
