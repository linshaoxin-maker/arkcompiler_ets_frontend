    ...
    varbinder::FunctionScope *const scope)
{
    ...
    scope->BindNode(func);
    func->SetIdent(id);
    VarBinder()->AsETSBinder()->AddInvokeFunctionThisParam(func);
...
ir::TypeNode *ETSChecker::GetPartialTypeBaseTypeNode(const ir::TSTypeParameterInstantiation *const typeParams)
{
...
{
    auto packageName = classDefProgram->GetPackageName().Mutf8();
