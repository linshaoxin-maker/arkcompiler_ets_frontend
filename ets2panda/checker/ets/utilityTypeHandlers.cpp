...
Type *ETSChecker::HandlePartialInterface(Type *typeToBePartial)
{
    ...
    auto *const partialInterDecl =
        CreatePartialTypeInterfaceDecl(partialClassName, qualifiedName, interfaceDecl, isClassDeclaredInCurrentFile);
                                                     ...
                                                     ir::TSInterfaceDeclaration *const newTSInterfaceDefinition)
{
    ...
    propClone->Variable()->SetScope(prop->IsStatic()
                                        ? newTSInterfaceDefinition->Scope()->AsClassScope()->StaticFieldScope()
                                        : newTSInterfaceDefinition->Scope()->AsClassScope()->InstanceFieldScope());
    ...
    ArenaMap<ir::TSTypeParameter *, ir::TSTypeParameter *> *likeSubstitution)
{
    ...
    auto *const newTypeParams =
        AllocNode<ir::TSTypeParameterDeclaration>(std::move(typeParams), thisTypeParams->RequiredParams());
                                                  ...
                                                  ir::TSInterfaceDeclaration *partialInterface)
{
    for (auto *const prop : interfaceDecl->Body()->Body()) {
        if (prop->IsMethodDefinition() && (prop->AsMethodDefinition()->Function()->IsGetter() ||
                                           prop->AsMethodDefinition()->Function()->IsSetter())) {
            auto *propMethod = prop->AsMethodDefinition();
            if (partialInterface->Scope()->FindLocal(propMethod->Id()->Name(),
                                                     varbinder::ResolveBindingOptions::VARIABLES) != nullptr) {
                continue;
            }
            auto *newProp = CreateNullishPropertyFromAccessorInInterface(propMethod, partialInterface);
            partialInterface->Scope()->AddBinding(Allocator(), nullptr, newProp->Variable()->Declaration(),
                                                  ScriptExtension::ETS);
...
ark::es2panda::ir::TSInterfaceDeclaration *ETSChecker::CreatePartialTypeInterfaceDecl(
    ...
    bool isClassDeclaredInCurrentFile)
{
    ...
    if (interfaceDecl->TypeParams() != nullptr) {
        newTypeParams = ProcessTypeParamAndGenSubstitution(interfaceDecl->TypeParams(), likeSubstitution);
    } else {
        ArenaVector<ir::TSTypeParameter *> typeParams(Allocator()->Adapter());
    ...
    for (auto *extend : interfaceDecl->Extends()) {
        auto *superPartialType = HandlePartialType(extend->Expr()->TsType())->AsETSObjectType();
        ...
        extends.push_back(AllocNode<ir::TSInterfaceHeritage>(superPartialRef));
    }
