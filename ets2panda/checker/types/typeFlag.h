...
enum class TypeFlag : uint64_t {
    ...
    ETS_TYPE = BYTE | SHORT | INT | LONG | FLOAT | DOUBLE | CHAR | ETS_BOOLEAN | ETS_VOID | ETS_OBJECT | ETS_ARRAY |
               WILDCARD | ETS_TYPE_PARAMETER | ETS_ENUM | ETS_STRING_ENUM | ETS_DYNAMIC_TYPE | ETS_UNION,
    ...
    ETS_ARRAY_INDEX = BYTE | SHORT | INT | FLOAT | DOUBLE,
    ...
    NULLISH = UNDEFINED | NULL_TYPE,
    ANY_OR_NULLISH = ANY | NULLISH,
    ...
    UNIT = LITERAL | UNIQUE_SYMBOL | NULLISH,
    GETTER_SETTER = GETTER | SETTER,
    CONDITION_EXPRESSION_TYPE = NULLISH | CONSTANT | ETS_OBJECT | BYTE | SHORT | INT | LONG | FLOAT | DOUBLE |
                                ETS_BOOLEAN | ETS_ARRAY | ETS_ENUM | ETS_STRING_ENUM
};
