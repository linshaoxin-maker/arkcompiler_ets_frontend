                                  ...
                                  checker::Signature *signature, bool isExternal)
{
    ...
    const auto *const lambdaObjectCtorFunc = lambdaBody[lambdaBody.size() - 2U]->AsMethodDefinition()->Function();
    const auto *const lambdaObjectInvokeFunc = lambdaBody[lambdaBody.size() - 1]->AsMethodDefinition()->Function();

    AddLambdaFunctionThisParam(lambdaObjectCtorFunc, isExternal);
    AddLambdaFunctionThisParam(lambdaObjectInvokeFunc, isExternal);
