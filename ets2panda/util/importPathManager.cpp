...
void ImportPathManager::AddToParseList(const StringView &resolvedPath, bool isDefaultImport)
{
    if (ark::os::file::File::IsDirectory(resolvedPath.Mutf8())) {
#ifdef USE_UNIX_SYSCALL
        DIR *dir = opendir(resolvedPath.Mutf8().c_str());
        if (dir == nullptr) {
            throw Error(ErrorType::GENERIC, "", "Cannot open folder: " + resolvedPath.Mutf8());
        }
        ...
        while ((entry = readdir(dir)) != nullptr) {
            ...
            std::string filePath = resolvedPath.Mutf8() + "/" + entry->d_name;
...
StringView ImportPathManager::AppendExtensionOrIndexFileIfOmitted(const StringView &path) const
{
    ...
    if (ark::os::file::File::IsDirectory(realPath.Mutf8())) {
        ...
        std::array<std::string, 2> supportedIndexFiles = {"index.ets", "index.ts"};
    ...
    std::array<std::string, 2> supportedExtensions = {".ets", ".ts"};
