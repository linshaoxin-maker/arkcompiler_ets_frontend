...
import { AutofixInfoSet } from './lib/Autofixer';
...
function makeDiag(category: ts.DiagnosticCategory, code: number, file: ts.SourceFile, start: number, length: number, messageText: string): ts.Diagnostic {
...
export function translateDiag(srcFile: ts.SourceFile, problemInfo: ProblemInfo): ts.Diagnostic {
  ...
  const severity = (problemInfo.severity === ProblemSeverity.ERROR ? ts.DiagnosticCategory.Error : ts.DiagnosticCategory.Warning);
...
export function runArkTSLinter(tsBuilderProgram: ArkTSProgram, reverseStrictBuilderProgram: ArkTSProgram,
  ...
  for (const fileToLint of srcFiles) {
    ...
    if (incrementalLinterState.isFileChanged(fileToLint)) {
      ...
      linter.problemsInfos.forEach((x) => currentDiagnostics.push(translateDiag(fileToLint, x)));
