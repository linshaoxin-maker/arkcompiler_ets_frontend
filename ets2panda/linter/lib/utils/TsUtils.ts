...
export class TsUtils {
  ...
  isTypedArray(tsType: ts.Type, allowTypeArrays: string[]): boolean {
    ...
    return (
      !!decl && TsUtils.isArkTSCollectionsClassOrInterfaceDeclaration(decl) && allowTypeArrays.includes(symbol.getName())
    );
  }

  isArray(tsType: ts.Type): boolean {
    return this.isGenericArrayType(tsType) || this.isReadonlyArrayType(tsType) || this.isTypedArray(tsType, TYPED_ARRAYS);
  }
