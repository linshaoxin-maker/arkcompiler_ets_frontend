...
export class TypeScriptLinter {
  ...
  private handleClassDeclaration(node: ts.Node): void {
    ...
    if (staticBlocksCntr > 1) {
      let autofix: Autofix[] | undefined = [];
      let autofixable = this.autofixesInfo.shouldAutofix(node, FaultID.MultipleStaticBlocks);
  ...
  private checkLimitedStdLib(node: ts.Identifier, symbol: ts.Symbol): void {
