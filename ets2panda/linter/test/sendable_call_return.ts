...
handle1(new NormalData()); // WARING, The inferred return type is the sendable class with 'non-sendable-type'

function handle2<T>(p:T) {
    return new SendableAnimal<T>;
}
handle2(new SendableData()); // OK
handle2(new NormalData()); // WARING, The inferred return type is the sendable class with 'non-sendable-type'
...
handle3(new NormalData()); // WARING, If there are more than one generic parameter, all must be valid.
...
handle4(new NormalData()); // WARING, If it is a union type, all must be valid.