...
static ir::Identifier *GenTmpIdentifier(checker::Type *type, ArenaAllocator *allocator, varbinder::Scope *scope)
{
    ir::Identifier *ident = Gensym(allocator);
    auto *const tmp_var = scope->AddDecl<varbinder::LetDecl, varbinder::LocalVariable>(allocator, ident->Name(),
                                                                                       varbinder::VariableFlags::LOCAL);
    tmp_var->SetTsType(type);
    ident->SetVariable(tmp_var);
    ident->SetTsType(tmp_var->TsType());
    return ident;
}
...
bool ExpandBracketsPhase::Perform(public_lib::Context *ctx, parser::Program *program)
{
    ...
    program->Ast()->TransformChildrenRecursively([parser, checker, allocator](ir::AstNode *ast) -> ir::AstNode * {
        ...
        auto *ident = GenTmpIdentifier(dim_type, allocator, scope);
