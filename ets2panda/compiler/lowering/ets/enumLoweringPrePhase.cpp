...
ir::MethodDefinition *CreateMethodGetValueOf(const util::StringView &enumName,
                                             ir::TSTypeParameterInstantiation *tpParInst, checker::ETSChecker *checker)
{
    ...
    auto *arg0Prop = CreateIdentifierRef(checker::ETSEnumType::GetLiteralArrayName(), checker);
    ...
    auto *callee = CreateIdentifierRef("enumValueOf", checker);
    ...
    auto *func = checker->AllocNode<ir::ScriptFunction>(
        ...
        ir::ScriptFunction::ScriptFunctionData {
            body, ir::FunctionSignature(nullptr, std::move(params), retType->Clone(checker->Allocator(), nullptr)),
    ...
    auto *key = checker->AllocNode<ir::Identifier>("getValueOf", checker->Allocator());
    func->SetIdent(key);
...
ir::MethodDefinition *CreateMethodCreate(const util::StringView &enumName, bool isIntEnum, checker::ETSChecker *checker)
{
    ...
    auto *func = checker->AllocNode<ir::ScriptFunction>(
        checker->Allocator(),
        ir::ScriptFunction::ScriptFunctionData {
            body, ir::FunctionSignature(nullptr, std::move(params), CreateETSTypeReference(enumName, checker)),
            ir::ScriptFunctionFlags::HAS_RETURN, ir::ModifierFlags::STATIC | ir::ModifierFlags::PRIVATE, false,
            Language(Language::Id::ETS)});

    auto *key = CreateIdentifierRef("create", checker);
    func->SetIdent(key);
    auto *value = checker->AllocNode<ir::FunctionExpression>(func);
...
ir::MethodDefinition *CreateMethodValues(const util::StringView &enumName, checker::ETSChecker *checker)
{
    ...
    auto *argProp = CreateIdentifierRef(checker::ETSEnumType::GetLiteralArrayName(), checker);
    ...
    auto *func = checker->AllocNode<ir::ScriptFunction>(
        checker->Allocator(),
        ir::ScriptFunction::ScriptFunctionData {
            body, ir::FunctionSignature(nullptr, std::move(params), nullptr), ir::ScriptFunctionFlags::HAS_RETURN,
            ir::ModifierFlags::STATIC | ir::ModifierFlags::PUBLIC, false, Language(Language::Id::ETS)});
...
ir::MethodDefinition *CreateConstructor(checker::ETSChecker *checker)
{
    ...
    auto *func = checker->AllocNode<ir::ScriptFunction>(
        ...
        ir::ScriptFunction::ScriptFunctionData {body, ir::FunctionSignature(nullptr, std::move(params), nullptr),
                                                ...
                                                ir::ModifierFlags::NONE, false, Language(Language::Id::ETS)});
...
ir::ClassProperty *CreateEnumConstantArray(const util::StringView &enumName, ArenaVector<ir::Expression *> &&elements,
                                           checker::ETSChecker *checker)
{
    auto *key = checker->AllocNode<ir::Identifier>(checker::ETSEnumType::GetLiteralArrayName(), checker->Allocator());
                                                   ...
                                                   int idx, checker::ETSChecker *checker)
{
    ...
    auto *calleeProp = CreateIdentifierRef("create", checker);
    auto *callee = checker->AllocNode<ir::MemberExpression>(calleeObj, calleeProp,
                                                            ir::MemberExpressionKind::PROPERTY_ACCESS, false, false);
...
ir::AstNode *CreateEnumClassFromEnumDeclaration(ir::TSEnumDeclaration *enumDecl, checker::ETSChecker *checker)
{
    ...
    body.push_back(CreateMethodGetValueOf(enumName, tpParInst, checker));
    ...
    auto *classDef = checker->AllocNode<ir::ClassDefinition>(checker->Allocator(), ident, std::move(body), defModif,
                                                             Language(Language::Id::ETS));
    if (isIntEnum) {
        classDef->SetSuper(CreateETSTypeReference(Signatures::BUILTIN_ENUM_INT_CLASS, checker));
    } else {
        classDef->SetSuper(CreateETSTypeReference(Signatures::BUILTIN_ENUM_STRING_CLASS, checker));
    }
