...
std::string const &TupleLowering::Name()
{
    static std::string const NAME = "tuple-lowering";
    return NAME;
}
...
bool TupleLowering::Postcondition(CompilerContext *const ctx, const parser::Program *const program)
{
    for (const auto &[_, ext_programs] : program->ExternalSources()) {
        (void)_;
