...
bool TopLevelStatements::Perform(public_lib::Context *ctx, parser::Program *program)
{
    ...
    for (auto &[package, extPrograms] : extSrcs) {
        ...
        globalClass.InitGlobalClass(extPrograms, &triggeringCCtorMethodsAndPrograms, false);
    }
    ...
    globalClass.InitGlobalClass(mainModule, &triggeringCCtorMethodsAndPrograms, true);
    return true;
}
