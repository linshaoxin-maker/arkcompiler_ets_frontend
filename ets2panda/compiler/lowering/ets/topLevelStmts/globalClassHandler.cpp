...
void GlobalClassHandler::InitGlobalClass(const ArenaVector<parser::Program *> &programs)
{
    ...
    for (auto program : programs) {
        ...
        if (program->IsEntryPoint() && !generateMain && !MainFunctionExists(program->Ast()->Statements())) {
            generateMain = true;
        }
    ...
    InitCallToCCTOR(programs.front(), statements, generateMain && topLevelStatementsExist);
}
