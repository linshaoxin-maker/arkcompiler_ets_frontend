...
std::string_view DefaultParameterLowering::Name()
{
    static std::string_view name = "DefaultParameterLowering";
    return name;
}
                                                                                    ...
                                                                                    CompilerContext *ctx)
{
    ...
    std::for_each(parentParams.begin(), parentParams.end(), [&typeParams, checker](ir::TSTypeParameter *par) {
        ...
        auto *defaultType = par->DefaultType() != nullptr
                                ...
                                : nullptr;
        typeParams.push_back(checker->AllocNode<ir::TSTypeParameter>(ident, constraint, defaultType));
    });
                                                                        ...
                                                                        CompilerContext *ctx)
{
    ...
    std::for_each(funcParam.begin(), funcParam.end(), [checker](ir::Expression *expr) {
        if (expr->IsETSParameterExpression()) {
            auto *typeAnnot = expr->AsETSParameterExpression()->TypeAnnotation()->Clone(checker->Allocator(), nullptr);
            typeAnnot->SetParent(expr);
            auto *parIdent = expr->AsETSParameterExpression()->Ident();
            parIdent->SetTsTypeAnnotation(typeAnnot);
        }
    });
                                                          ...
                                                          ArenaVector<ir::Expression *> funcCallArgs)
{
    ...
    if ((method->Function()->ReturnTypeAnnotation() != nullptr) ||
        ...
        if ((method->Function()->ReturnTypeAnnotation() != nullptr) &&
            ...
            stmt = checker->AllocNode<ir::ReturnStatement>(checker->AllocNode<ir::ThisExpression>());
        } else {
    ...
    ArenaVector<ir::Expression *> funcCallArgs)
{
    ...
    funcNode = checker->AllocNode<ir::ScriptFunction>(std::move(signature), body, method->Function()->Flags(), false,
                                                      ...
                                                      CompilerContext *ctx)
{
    ...
    auto *const overloadMethod =
        checker->AllocNode<ir::MethodDefinition>(ast->AsMethodDefinition()->Kind(), funcExpression->Function()->Id(),
                                                 funcExpression, method->Modifiers(), checker->Allocator(), false);
    ...
    for (auto [it, i] = std::tuple {params.rbegin(), 1}; it != params.rend(); ++it, i++) {
        ...
        std::for_each(
            params.begin(), params.end() - i, [&funcCallArgs, &funcDefinitionArgs, checker](ir::Expression *expr) {
                ...
                // That is already declared in function  arguments scope.
                // So need set Reference to avoid problem with variable
                funcArg->SetReference();
                ...
                auto *ident =
                    expr->AsETSParameterExpression()->Ident()->Clone(checker->Allocator(), nullptr)->AsIdentifier();
                auto *funcParam = checker->Allocator()->New<ir::ETSParameterExpression>(ident->AsIdentifier(), nullptr);

                // it seems that Clone() didn't copy TypeAnnotaion section ..
                // and this causes a problem later on the chcker phase, so set it explicitly
                funcParam->AsETSParameterExpression()->Ident()->SetTsTypeAnnotation(
