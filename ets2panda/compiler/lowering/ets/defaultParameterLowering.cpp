    ...
    ArenaVector<ir::Expression *> funcCallArgs)
{
    ...
    ir::ScriptFunction *funcNode = checker->AllocNode<ir::ScriptFunction>(
        std::move(signature), body, method->Function()->Flags(), false, method->Function()->Language());
    ...
    for (auto [it, i] = std::tuple {params.rbegin(), 1}; it != params.rend(); ++it, i++) {
        ...
        std::for_each(
            params.begin(), params.end() - i, [&funcCallArgs, &funcDefinitionArgs, checker](ir::Expression *expr) {
                ...
                funcCallArgs.push_back(std::move(funcArg));
                ...
                funcDefinitionArgs.push_back(std::move(funcParam));
            });
