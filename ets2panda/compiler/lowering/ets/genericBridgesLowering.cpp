                                           ...
                                           ir::ScriptFunction const *const derivedFunction) const
{
    ...
    checker::ETSFunctionType *bridgeMethodType = checker->BuildFunctionSignature(bridgeMethod->Function());
                                                ...
                                                Substitutions const &substitutions) const
{
    ...
    auto const *derivedSignature =
        derivedMethod->Function()->Signature()->Substitute(relation, substitutions.derivedConstraints);

    if (overrides(baseSignature1, derivedSignature)) {
    ...
    if (overrides(derivedSignature, baseSignature2)) {
        derivedFunction = derivedMethod->Function();
    }

    for (auto *const overload : derivedMethod->Overloads()) {
        derivedSignature = overload->Function()->Signature()->Substitute(relation, substitutions.derivedConstraints);
        if (overrides(baseSignature1, derivedSignature)) {
            //  NOTE: we already have custom-implemented method with the required bridge signature.
            //  Probably sometimes we will issue warning notification here...
            return;
        ...
        if (derivedFunction == nullptr && overrides(derivedSignature, baseSignature2)) {
            ...
            derivedFunction = overload->Function();
        }
    }
    ...
    for (auto *item : superDefinition->Body()) {
        if (item->IsMethodDefinition()) {
            ...
            if (method->Kind() != ir::MethodDefinitionKind::METHOD || method->IsStatic() || method->IsFinal() ||
                method->IsAbstract() || method->Id()->Name().Utf8().find("lambda$invoke$") != std::string_view::npos) {
                continue;
            }
