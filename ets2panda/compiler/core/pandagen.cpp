...
void PandaGen::DefineGetterSetterByValue(const ir::AstNode *node, std::tuple<VReg, VReg, VReg, VReg> registers,
                                         bool setName)
{
    constexpr auto objIndex = 0u;
    constexpr auto nameIndex = 1u;
    constexpr auto getterIndex = 2u;
    constexpr auto setterIndex = 3u;
    LoadConst(node, setName ? Constant::JS_TRUE : Constant::JS_FALSE);
    Ra().Emit<EcmaDefinegettersetterbyvalue>(node, std::get<objIndex>(registers), std::get<nameIndex>(registers),
                                             std::get<getterIndex>(registers), std::get<setterIndex>(registers));
}
