...
static auto source = std::string("function foo() {}");
...
int main(int argc, char **argv)
{
    ...
    std::cout << impl->AstNodeDumpJSONConst(context, statements[0]) << std::endl;
    auto classDeclaration = statements[0];
    auto classDefinition = impl->ClassDeclarationDefinition(context, classDeclaration);

    auto class_statements = impl->ClassDefinitionBody(context, classDefinition, &n);
    auto funcExpr = impl->MethodDefinitionFunction(context, class_statements[0]);
    std::cout << impl->AstNodeDumpJSONConst(context, funcExpr) << std::endl;
    std::cout << "IsScriptFucntion: " << impl->IsScriptFunction(funcExpr) << std::endl;
    auto flags = impl->ScriptFunctionFlagsConst(context, funcExpr);
    std::cout << "scrFunc flags: " << flags << std::endl;

    while (flags) {
        std::cout << (flags & 1U);
        flags = (Es2pandaScriptFunctionFlags)(flags >> 1);
    }
    std::cout << std::endl;
