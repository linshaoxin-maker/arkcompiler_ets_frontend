...
TEST_F(ASTVerifierTest, CheckReadonly)
{
    ...
    checks.insert("CheckReadonlyFieldsForAll");
    const auto &messages = verifier.Verify(ast, checks);

    ASSERT_EQ(messages.size(), 1);
    auto invariant = messages[0].Invariant();
    auto cause = messages[0].Cause();
    ASSERT_EQ(invariant, "CheckReadonlyFieldsForAll");
    ASSERT_EQ(cause, "Readonly field cannot be const");
