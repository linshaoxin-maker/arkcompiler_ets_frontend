...
TEST_F(ASTVerifierTest, NullParent)
{
    ...
    StringLiteral empty_node;

    const auto check = "NodeHasParent";
    auto checks = ASTVerifier::InvariantSet {};
    checks.insert(check);
    const auto [warnings, errors] = verifier.Verify({{"NodeHasParent"}}, {{}}, &empty_node, checks);
    bool has_parent = warnings.empty();
    ASSERT_FALSE(has_parent);
...
TEST_F(ASTVerifierTest, NullType)
{
    ...
    auto checks = ASTVerifier::InvariantSet {};
    checks.insert(check);
    const auto [warnings, errors] = verifier.Verify({{"NodeHasType"}}, {{}}, &empty_node, checks);
    bool has_type = warnings.empty();
...
TEST_F(ASTVerifierTest, WithoutScope)
{
    ...
    const auto [warnings, errors] = verifier.Verify({{"VariableHasScope"}}, {{}}, &empty_node, checks);
...
TEST_F(ASTVerifierTest, ScopeTest)
{
    ...
    FunctionScope parent_scope(Allocator(), nullptr);
...
TEST_F(ASTVerifierTest, ArithmeticExpressionCorrect1)
{
    ...
    auto arithmetic_expression = BinaryExpression(&left, &right, TokenType::PUNCTUATOR_PLUS);
    ...
    const auto [warnings, errors] =
        verifier.Verify({{"ArithmeticOperationValid"}}, {{}}, arithmetic_expression.AsBinaryExpression(), checks);
    ASSERT_EQ(warnings.size(), 0);
}
...
TEST_F(ASTVerifierTest, ArithmeticExpressionCorrect2)
{
    ...
    auto arithmetic_expression = BinaryExpression(&left1, &right1, TokenType::PUNCTUATOR_PLUS);
...
TEST_F(ASTVerifierTest, ArithmeticExpressionNegative1)
{
    ...
    const StringView left_param("1");
    constexpr uint32_t RIGHT_PARAM = 1;
    auto left = StringLiteral(left_param);
    auto right = NumberLiteral(Number {RIGHT_PARAM});
    auto arithmetic_expression = BinaryExpression(&left, &right, TokenType::PUNCTUATOR_DIVIDE);
...
TEST_F(ASTVerifierTest, SequenceExpressionType)
{
    ...
    auto *sequence_expression = Tree(Node<SequenceExpression>(
    ...
    const auto [warnings, errors] =
        verifier.Verify({{"SequenceExpressionHasLastType"}}, {{}}, sequence_expression, checks);
