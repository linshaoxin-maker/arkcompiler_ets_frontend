                                                       ...
                                                       bool ignoreCallExpression, bool *isChainExpression)
{
    ...
    while (true) {
        ...
        switch (tokenType) {
            ...
            case lexer::TokenType::LITERAL_IDENT: {
                if (auto *asExpression = ParsePotentialAsExpression(returnExpression); asExpression != nullptr) {
                    return asExpression;
                }
                break;
            }
            case lexer::TokenType::PUNCTUATOR_LEFT_SHIFT:
            case lexer::TokenType::PUNCTUATOR_LESS_THAN: {
                if (ParsePotentialGenericFunctionCall(primaryExpr, &returnExpression, startLoc, ignoreCallExpression)) {
                    break;
                }
                continue;
            }
            case lexer::TokenType::PUNCTUATOR_BACK_TICK: {
                returnExpression = ParsePostPrimaryExpressionBackTick(returnExpression, startLoc);
                continue;
            }
            case lexer::TokenType::PUNCTUATOR_LEFT_PARENTHESIS: {
            ...
            case lexer::TokenType::PUNCTUATOR_EXCLAMATION_MARK: {
                if (ParsePotentialNonNullExpression(&returnExpression, startLoc)) {
                    break;
                }
                continue;
            }
            default: {
                if (auto tmp = ParsePostPrimaryExpressionDot(returnExpression, tokenType, isChainExpression);
                    tmp != nullptr) {
