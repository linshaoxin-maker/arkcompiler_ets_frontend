...
public:
    ETSParser(Program *program, const CompilerOptions &options, ParserStatus status = ParserStatus::NO_OPTS)
        : TypedParser(program, options, status),
          globalProgram_(GetProgram()),
          pathResolver_(ArkTSConfig(), options.stdLib, Extension()),
...
private:
    ...
    std::vector<Program *> ParseSources(std::vector<std::string> paths);
    ...
    std::pair<ArenaVector<ir::ETSImportDeclaration *>, std::vector<std::string>> ParseImportDeclarations();
    ...
    ir::Statement *ParseReExport(lexer::SourcePosition startLoc);
    ...
    Value ValidatePathResolver(util::ImportPathResolver::ResultT<Value> result)
    {
        if (result.IsError()) {
            const auto &[code, error] = result.ExpectError();
            using ErrorCode = util::ImportPathResolver::ErrorCode;
            switch (code) {
                case ErrorCode::ARKTS_PREFIX_FAIL:
                    ThrowSyntaxError({"Can't find prefix for '", error, "' in ", ArkTSConfig()->ConfigPath()});
                case ErrorCode::INCORRECT_PATH:
                    ThrowSyntaxError("Incorrect path: " + error);
                default:
                    UNREACHABLE();
            }
        }
    ...
    {
        return pathResolver_.IsStdLib(GetProgram());
    }
...
public:
    ...
    {
        return pathResolver_.ResolvedParsedSourcesMap();
    }

private:
    parser::Program *globalProgram_;
    util::ImportPathResolver pathResolver_;
    ArenaVector<ir::ETSReExportDeclaration *> reExports_;
